...
class Dept:
    ...
    def __init__(self, fwindow) -> None:
        # importing file
        # self.sw=pwindow
        # self.sw=Tk()

        self.save = None
        self.mw = Toplevel(fwindow)
        ...
        self.head1 = Label(self.mw, text="Add Department", font=('Arial', 31, 'bold'), bg='#d7948c', relief='groove',
                           width=50)

        self.l1 = Label(self.mw,text="Department Name")
        self.t1 = Entry(self.mw)
        self.l2 = Label(self.mw,text="Head Of Department")
    ...
    def saveData(self):
        try:
            myqry = "insert into department values(%s,%s)"
            rowcount = self.curr.execute(myqry,
                                         (self.t1.get(), self.t2.get()))
            self.conn.commit()
            if rowcount == 1:
                messagebox.showinfo("Success", "Department Record Succesfully Updated", parent=self.mw)
                self.clearPage()
            else:
    ...
    def updatePage(self):
        ...
        try:
            myqry = "update department set Hod = %s where Dname = %s"
            rowcount = self.curr.execute(myqry, (self.t2.get(),self.t1.get()))
            self.conn.commit()
            if rowcount == 1:
                messagebox.showinfo("Sucess", "Department Record Updated in DataBase Successfully", parent=self.mw)
                self.clearPage()
            else:
    ...
    def deleteData(self):
        ...
        if ans == "yes":
            self.databaseConnection()
            try:
                myqry = " delete from department where Dname=%s"
                rowcount = self.curr.execute(myqry, (self.t1.get()))
                self.conn.commit()
                if rowcount == 1:
                    messagebox.showinfo("Success", "Department Record Deleted Successfully", parent=self.mw)
                    self.clearPage()
                else:
                    messagebox.showwarning("Failure", "Check all Fields Carefully", parent=self.mw)
            except Exception as e:
    ...
    def fetchData(self, col=None):
        ...
        self.databaseConnection()
        try:
            myqry = "select * from department where Dname=%s"
            rowcount = self.curr.execute(myqry, (deptn))
    ...
    def searchData(self):
        self.databaseConnection()
        try:
            myqry = "select * from department where Hod like %s"
