...
class stud:
    ...
    def __init__(self, pwindow) -> None:
        # importing file
        # self.sw=pwindow
        # self.sw=Tk()
        ...
        self.bimg1 = Image.open("projectimages//spage.png")
        ...
        self.head1 = Label(self.mw, text="Add Student", font=('Arial', 31, 'bold'), bg='#d7948c', relief='groove',
                           width=50)
        ...
        self.v2 = StringVar()
        self.l7 = Label(self.mw, text="Department")
        ...
        self.v3 = StringVar()
        self.l8 = Label(self.mw, text="Courses")
        ...
        self.mytable.heading('c8', text='Courses')
    ...
    def saveData(self):
        ...
        try:
            myqry = "insert into student values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            ...
            self.conn.commit()
            if rowcount == 1:
                messagebox.showinfo("Success", "Student Record Succesfully Updated", parent=self.mw)
                self.clearPage()
            else:
    ...
    def fetchData(self, col=None):
        ...
        self.databaseConnection()
        try:
            myqry = "select * from student where RollNo=%s"
            rowcount = self.curr.execute(myqry, (RollNo))
            ...
            if data:
                ...
                self.img1= Image.open("student_img//" + self.actualname)
                self.img1 = self.img1.resize((130,130))
                self.img2 = ImageTk.PhotoImage(self.img1)
                self.imglbl.config(image=self.img2)
                ...
                messagebox.showwarning("Empty", "No Student Found for this RollNo", parent=self.mw)
        except Exception as e:
    ...
    def updatePage(self):
        ...
        if self.actualname == self.oldname:
            ...
            if self.oldname==self.default_img_name:
                ...
                import os
                os.remove("student_img//"+self.oldname)

        self.databaseConnection()
        try:
            myqry = "update student set Name = %s,Phone = %s,Gender = %s," \
                    "DOB = %s, Address = %s,Department = %s,Courses = %s,Pic = %s where RollNo=%s"
            rowcount = self.curr.execute(myqry, (self.t2.get(), self.t3.get(), self.v1.get(), self.t5.get_date(),
                                                 self.t6.get("1.0", END), self.v2.get(), self.v3.get(),self.actualname,self.t1.get()))
            self.conn.commit()
            if rowcount == 1:
    ...
    def deleteData(self):
        ...
        if ans == "yes":
            ...
            self.databaseConnection()
            try:
                myqry = " delete from student where RollNo=%s"
                rowcount = self.curr.execute(myqry, (self.t1.get()))
                self.conn.commit()
                if rowcount == 1:
                    messagebox.showinfo("Success", "Student Data Deleted Successfully", parent=self.mw)
                    self.clearPage()
                else:
                    messagebox.showwarning("Failure", "Check all Fields Carefully", parent=self.mw)
            except Exception as e:
    ...
    def searchData(self):
        self.databaseConnection()
        try:
            myqry = "select * from student where Name like %s"
    ...
    def getallCourses(self):
        self.databaseConnection()
        try:
            myqry = "select *  from course where Dname=%s "
