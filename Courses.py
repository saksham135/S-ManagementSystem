...
class Cour:
    ...
    def __init__(self, xwindow) -> None:
        self.save = None
        self.mw = Toplevel(xwindow)
        self.mw.title("Add Courses")
        ...
        self.head1 = Label(self.mw, text="Add Course", font=('Arial', 31, 'bold'), bg='#d7948c', relief='groove',
                           width=50)

        self.l1 = Label(self.mw, text="Department")

        self.l2 = Label(self.mw, text="Course Name")

        self.l3 = Label(self.mw, text="Duration")

        self.l4 = Label(self.mw, text="Fees")


        self.v1 = StringVar()
        self.c1 = Combobox(self.mw,values=('IT', 'Commerce','Science',),textvariable=self.v1)
        ...
        self.mytable.heading('c4', text='Fees')
    ...
    def saveData(self):
        ...
        try:
            myqry = "insert into course values(%s,%s,%s,%s)"
            rowcount = self.curr.execute(myqry,(self.v1.get(), self.t2.get(),self.t3.get(),self.t4.get()))
            self.conn.commit()
            if rowcount == 1:
                messagebox.showinfo("Success", "Course Record Succesfully Updated", parent=self.mw)
                self.clearPage()
            else:
    ...
    def updatePage(self):
        self.databaseConnection()
        try:
            myqry = "update course set Dname = %s,Duration = %s , Fee=%s where Cname = %s"
            rowcount = self.curr.execute(myqry, (self.v1.get(),self.t3.get(),self.t4.get(),self.t2.get()))
            self.conn.commit()
            if rowcount == 1:
                messagebox.showinfo("Sucess", "Course Record Updated in DataBase Successfully", parent=self.mw)
                self.clearPage()
            else:
    ...
    def deleteData(self):
        ...
        if ans == "yes":
            self.databaseConnection()
            try:
                myqry = " delete from course where Cname=%s"
                rowcount = self.curr.execute(myqry, (self.t2.get()))
                self.conn.commit()
                if rowcount == 1:
                    messagebox.showinfo("Success", "Course Record Deleted Successfully", parent=self.mw)
                    self.clearPage()
                else:
                    messagebox.showwarning("Failure", "Check all Fields Carefully", parent=self.mw)
            except Exception as e:
    ...
    def fetchData(self, col=None):
        ...
        self.databaseConnection()
        try:
            myqry = "select * from course where Cname=%s"
            rowcount = self.curr.execute(myqry, (course))
            ...
            if data:
                ...
                messagebox.showwarning("Empty", "No Course Record Found for this Course", parent=self.mw)
    ...
    def searchData(self):
        ...
        try:
            if self.v1.get()=='Choose Department':
                ...
                dpt=self.v1.get()
            myqry = "select * from course where Dname like %s"
            rowcount = self.curr.execute(myqry,(dpt+"%"))
            ...
            if data:
                ...
                messagebox.showwarning("Empty", "No Course Found", parent=self.mw)
        except Exception as e:
            messagebox.showerror("Query Error", "Error while Insertion : \n" + str(e), parent=self.mw)
